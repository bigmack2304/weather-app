...
function City5d3hWeather_dayWeather({ dataIdRender, sorted_days_weather, weather }: TProps) {
    ...
    useEffect(() => {
        ...
        for (let week of sorted_days_weather) {
            if (!temp.has(week[0].dt)) {
                temp.set(week[0].dt, createRef());
            }
        }

        setDesctopListRef(temp);
    }, [sorted_days_weather]);
            ...
            {!is_mobile_screen_size() ? (
                <>
                    {desctop_html(
                        ...
                        calc_sunrise,
                        calc_sunset,
                        desctopListRef as Map<number, React.RefObject<HTMLDivElement>>
                    )}
                </>
            ) : (
...
function desctop_html(
    ...
    calc_sunrise: (val: number, val2: number) => number,
    calc_sunset: (val: number, val2: number) => number,
    desctopListRef: Map<number, React.RefObject<HTMLDivElement>>
) {
    return (
        <TransitionGroup component={null}>
            <CSSTransition
                timeout={250}
                classNames={"City5d3hWeather__day_time_list"}
                key={sorted_days_weather[day_id][0].dt}
                nodeRef={desctopListRef.get(sorted_days_weather[day_id][0].dt)}
            >
                <div className="City5d3hWeather__day_time_list" ref={desctopListRef.get(sorted_days_weather[day_id][0].dt)}>
                    {sorted_days_weather[day_id].map((forecast) => {
                        return (
                            <div key={forecast.dt} className="City5d3hWeather__day_time_weather">
                                <span className="City5d3hWeather__day_time_time">
