import * as fetch_current_weather from "./fetch_current_weather";
...
describe("test fetch_current_weather.ts", () => {
    ...
    test("test fetch current adress", async () => {
        ...
        await fetch_current_weather.fetch_current_weather({ callBack: fn_on_response, errorCallback: fn_on_error, lat: 0, lon: 0 });
        expect(get_adress).toBe(
            "https://api.openweathermap.org/data/2.5/weather?lat=0&lon=0&units=metric&lang=en&appid=0000c0000c0000c0000"
        );
    });
